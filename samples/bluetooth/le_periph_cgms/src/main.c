...
static void on_key_received(uint8_t conidx, uint32_t metainfo, const gapc_pairing_keys_t *p_keys)
{
	...
	/* Save under the key "ble/bond_data_0" */
	err = settings_save_one("ble/bond_keys_0", &stored_keys, sizeof(gapc_pairing_keys_t));
	if (err) {
				...
				bool enc_key_present, uint8_t key_type)
{
	...
	err = settings_save_one("ble/bond_data_0", &bond_data_saved, sizeof(gapc_bond_data_t));
	if (err) {
		LOG_ERR("Failed to store test_data (err %d)", err);
	}
			...
			settings_read_cb read_cb, void *cb_arg)
{
	...
	if (strcmp(name, "bond_keys_0") == 0) {
		...
		return 0;
	} else if (strcmp(name, "bond_data_0") == 0) {
